// ============================================
// VERSÃO V13 - COPIAR NO SUPABASE DASHBOARD
    ...
    switch (command) {
      case 'saldo':
        ...
        const totalBalance = accounts?.reduce((sum, acc) => 
          sum + parseFloat(acc.current_balance || '0'), 0) || 0;
      ...
      case 'metas':
        ...
        const { data: goals } = await supabase
          .from('financial_goals') // ✅ CORRIGIDO: era 'goals'
          .select('name, target_amount, current_amount, deadline, status, goal_type') // ✅ CORRIGIDO: era 'target_date'
          .eq('user_id', user.id)
          .eq('status', 'active')
          .eq('goal_type', 'savings') // ✅ NOVO: filtra só metas de economia
          .order('deadline', { ascending: true }) // ✅ CORRIGIDO: era 'target_date'
          .limit(5);
      ...
      case 'investimento':
        ...
        if (!investments || investments.length === 0) {
          ...
          const totalInvested = investments.reduce((sum, inv) => 
            sum + parseFloat(inv.total_invested || 0), 0);
          const totalCurrent = investments.reduce((sum, inv) => 
            sum + parseFloat(inv.current_value || 0), 0);
      ...
      case 'cartao':
        ...
        const { data: cards } = await supabase
          .from('credit_cards')
          .select('id, name, last_four_digits, due_day, credit_limit') // ✅ CORRIGIDO: era 'last_four'
          .eq('user_id', user.id)
          .eq('is_active', true)
          .eq('is_archived', false); // ✅ NOVO: exclui arquivados
