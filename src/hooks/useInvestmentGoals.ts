...
const goalsCacheLight = new Map<string, InvestmentGoalWithMetrics[]>();
const goalsCacheFull = new Map<string, InvestmentGoalWithMetrics[]>();
const goalsRequests = new Map<string, Promise<InvestmentGoalWithMetrics[]>>();

const goalsRequestKey = (userId: string, lightweight: boolean) =>
  `${userId}:${lightweight ? 'light' : 'full'}`;

function invalidateGoalsCaches(userId: string) {
  goalsCacheLight.delete(userId);
  goalsCacheFull.delete(userId);
  goalsRequests.delete(goalsRequestKey(userId, true));
  goalsRequests.delete(goalsRequestKey(userId, false));
}
...
function buildLightweightEnrichedGoal(
  goal: InvestmentGoal,
  ...
  const monthsTotal = calculateMonthsTotal(goal.start_date, goal.target_date);
  const monthsElapsed = calculateMonthsElapsed(goal.start_date);
  const monthsRemaining = calculateMonthsRemaining(goal.target_date);
  const monthly = Number(goal.monthly_contribution || 0);
  const totalContributions = monthly * monthsRemaining;
  const naiveProjection = effectiveCurrentAmount + totalContributions;
  const finalProjection = naiveProjection;
  ...
  const metrics: InvestmentGoalPortfolioMetrics = {
    ...
    is_on_track: finalProjection >= target,
    shortfall: Math.max(0, Number((target - finalProjection).toFixed(2))),
    ...
    current_gap: Math.max(0, Number((target - effectiveCurrentAmount).toFixed(2))),
    projected_gap: Math.max(0, Number((target - finalProjection).toFixed(2))),
  };
...
export interface UseInvestmentGoalsOptions {
  /** Sem RPCs por meta; suficiente para vínculos na carteira e defaults de planejamento. */
  lightweight?: boolean;
}

export function useInvestmentGoals(options?: UseInvestmentGoalsOptions) {
  const lightweight = options?.lightweight ?? false;
  const { user, loading: authLoading } = useAuth();

  const [goals, setGoals] = useState<InvestmentGoalWithMetrics[]>(() => {
    if (!user?.id) return [];
    const m = lightweight ? goalsCacheLight : goalsCacheFull;
    return m.get(user.id) ?? [];
  });
  const [loading, setLoading] = useState(() => {
    if (!user?.id) return true;
    const m = lightweight ? goalsCacheLight : goalsCacheFull;
    return !m.has(user.id);
  });
  ...
  const getGoalProjection = useCallback(
    async (
      ...
      try {
        ...
        const { data, error: rpcError } = await supabase.rpc('calculate_investment_projection', {
  ...
  const getRpcGoalMetrics = useCallback(async (goalId: string): Promise<InvestmentGoalMetrics | null> => {
    try {
      const { data, error: rpcError } = await supabase.rpc('get_investment_goal_metrics', {
        p_goal_id: goalId,
      });
  ...
  const enrichGoal = useCallback(
    async (goal: InvestmentGoal, activeInvestments: Investment[]): Promise<InvestmentGoalWithMetrics> => {
      const linkedInvestmentDetails: LinkedInvestmentSnapshot[] = activeInvestments
        .filter((investment) => goal.linked_investments?.includes(investment.id))
        .map((investment) => ({
          id: investment.id,
  ...
  const fetchGoals = useCallback(async () => {
    ...
    const key = goalsRequestKey(user.id, lightweight);
    const targetCache = lightweight ? goalsCacheLight : goalsCacheFull;
    const cached = targetCache.get(user.id);
    ...
    try {
      ...
      const existingRequest = goalsRequests.get(key);
      if (existingRequest) {
      ...
      const request = (async (): Promise<InvestmentGoalWithMetrics[]> => {
        ...
        const enrichedGoals = lightweight
          ? (rawGoals || []).map((goal) => buildLightweightEnrichedGoal(goal, activeInvestments || []))
          : await Promise.all(
              (rawGoals || []).map((goal) => enrichGoal(goal, activeInvestments || [])),
            );

        targetCache.set(user.id, enrichedGoals);
        return enrichedGoals;
      })();

      goalsRequests.set(key, request);
      ...
      goalsRequests.delete(key);
      setLoading(false);
    }
  }, [authLoading, user?.id, lightweight, enrichGoal]);

  const createGoal = useCallback(
    async (input: CreateInvestmentGoalInput): Promise<InvestmentGoal | null> => {
      try {
        const {
          data: { user: authUser },
        ...
        invalidateGoalsCaches(authUser.id);
        await fetchGoals();
  ...
  const updateGoal = useCallback(
    async (id: string, input: UpdateInvestmentGoalInput): Promise<boolean> => {
      try {
        ...
        if (authUser) invalidateGoalsCaches(authUser.id);
  ...
  const deleteGoal = useCallback(async (id: string): Promise<boolean> => {
    try {
      const { error: deleteError } = await supabase.from('investment_goals').delete().eq('id', id);
      ...
      toast.error(err instanceof Error ? err.message : 'Erro ao deletar meta');
      return false;
    }
  }, []);

  const getGoalMetrics = useCallback(
    async (goalId: string): Promise<InvestmentGoalPortfolioMetrics | null> => {
  ...
  const getGoalWithMetrics = useCallback(
    async (goalId: string): Promise<InvestmentGoalWithMetrics | null> => {
  ...
  const addContribution = useCallback(
    async (input: CreateContributionInput): Promise<boolean> => {
      try {
        const {
          data: { user: authUser },
        } = await supabase.auth.getUser();
        ...
        const { error } = await supabase.from('investment_goal_contributions').insert({
          user_id: authUser.id,
        ...
        toast.success(
          `Aporte de R$ ${input.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} registrado!`,
        );
        return true;
      } catch (err: unknown) {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'investment_goals' }, () => {
        if (user?.id) invalidateGoalsCaches(user.id);
        void fetchGoals();
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'investment_goal_contributions' }, () => {
        if (user?.id) invalidateGoalsCaches(user.id);
        void fetchGoals();
      })
  ...
  }, [authLoading, fetchGoals, user?.id]);
