...
import { Bell, Clock, Calendar, TrendingUp, Target, Sparkles, Save } from 'lucide-react';
...
export function NotificationsSettings() {
  ...
  // Modo Não Perturbe
  const [dndEnabled, setDndEnabled] = useState(notificationPreferences?.dnd_enabled ?? false);
  const [dndStartTime, setDndStartTime] = useState(notificationPreferences?.dnd_start_time || '22:00');
  const [dndEndTime, setDndEndTime] = useState(notificationPreferences?.dnd_end_time || '08:00');
  ...
  const [weeklySummaryDay, setWeeklySummaryDay] = useState(notificationPreferences?.weekly_summary_day || 0);
  ...
  const [monthlySummaryDay, setMonthlySummaryDay] = useState(notificationPreferences?.monthly_summary_day || 1);
  ...
  const [billReminderDays, setBillReminderDays] = useState(notificationPreferences?.bill_reminder_days_before || 3);
  
  const [budgetAlertsEnabled, setBudgetAlertsEnabled] = useState(notificationPreferences?.budget_alerts_enabled ?? true);
  const [budgetAlertThreshold, setBudgetAlertThreshold] = useState(notificationPreferences?.budget_alert_threshold || 80);
            ...
            {weeklySummaryEnabled && (
              <div className="grid grid-cols-2 gap-4 pl-4">
                <div className="space-y-2">
                  ...
                  <Select value={String(weeklySummaryDay)} onValueChange={(val) => setWeeklySummaryDay(Number(val))}>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            {billRemindersEnabled && (
              <div className="pl-4">
                <Label htmlFor="billDays">Dias antes do vencimento</Label>
                <Input
                  id="billDays"
                  type="number"
                  min={1}
                  ...
                  onChange={(e) => setBillReminderDays(Number(e.target.value))}
                  className="max-w-[200px]"
                />
              </div>
            )}
          ...
          <div className="space-y-3">
            ...
            {budgetAlertsEnabled && (
              <div className="pl-4">
                <Label htmlFor="budgetThreshold">Limite de alerta (%)</Label>
                <Input
                  id="budgetThreshold"
                  type="number"
                  min={50}
          ...
          <div className="space-y-3">
            ...
            {anaTipsEnabled && (
              <div className="pl-4">
                ...
                <Select value={anaTipsFrequency} onValueChange={setAnaTipsFrequency}>
                  <SelectTrigger id="anaTipsFreq" className="max-w-[200px]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
