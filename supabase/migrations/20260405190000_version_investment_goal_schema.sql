...
CREATE INDEX IF NOT EXISTS idx_investment_goals_user
  ON public.investment_goals (user_id);

CREATE INDEX IF NOT EXISTS idx_investment_goals_status
  ON public.investment_goals (status)
  WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_investment_goals_target_date
  ON public.investment_goals (target_date);

CREATE INDEX IF NOT EXISTS idx_investment_goals_linked_investments
  ON public.investment_goals
  USING gin (linked_investments);

CREATE INDEX IF NOT EXISTS idx_investment_contributions_goal
  ON public.investment_goal_contributions (goal_id);

CREATE INDEX IF NOT EXISTS idx_investment_contributions_user
  ON public.investment_goal_contributions (user_id);

CREATE INDEX IF NOT EXISTS idx_investment_contributions_date
  ON public.investment_goal_contributions (date DESC);

CREATE INDEX IF NOT EXISTS idx_investment_alerts_user_active
  ON public.investment_alerts (user_id, is_active);

CREATE INDEX IF NOT EXISTS idx_investment_alerts_ticker
  ON public.investment_alerts (ticker)
  WHERE is_active = TRUE;

CREATE INDEX IF NOT EXISTS idx_investment_alerts_active
  ON public.investment_alerts (is_active, last_checked);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'investment_goals'
      AND policyname = 'investment_goals_select_own'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'investment_goals'
      AND policyname = 'investment_goals_insert_own'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'investment_goals'
      AND policyname = 'investment_goals_update_own'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'investment_goals'
      AND policyname = 'investment_goals_delete_own'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'investment_goal_contributions'
      AND policyname = 'contributions_select_own'
  ) THEN
    CREATE POLICY contributions_select_own
      ON public.investment_goal_contributions
      FOR SELECT
      USING (auth.uid() = user_id);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'investment_goal_contributions'
      AND policyname = 'contributions_insert_own'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'investment_goal_contributions'
      AND policyname = 'contributions_delete_own'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'investment_alerts'
      AND policyname = 'Users can view own alerts'
  ) THEN
    CREATE POLICY "Users can view own alerts"
      ON public.investment_alerts
      FOR SELECT
      USING (auth.uid() = user_id);
  END IF;
...
BEGIN
  ...
  FROM public.calculate_investment_projection(
  ...
  RETURN jsonb_build_object(
    ...
    'shortfall', CASE
      WHEN COALESCE(v_final_projection, v_goal.current_amount) < v_goal.target_amount
        THEN ROUND(v_goal.target_amount - COALESCE(v_final_projection, v_goal.current_amount), 2)
      ELSE 0
    END
  );
END;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE trigger_schema = 'public'
      AND event_object_table = 'investment_alerts'
      AND trigger_name = 'trigger_update_investment_alerts_timestamp'
  ) THEN
    CREATE TRIGGER trigger_update_investment_alerts_timestamp
      BEFORE UPDATE ON public.investment_alerts
      FOR EACH ROW
      EXECUTE FUNCTION public.update_investment_alerts_updated_at();
  END IF;
  ...
  IF NOT EXISTS (
    ...
    FROM information_schema.triggers
    WHERE trigger_schema = 'public'
      AND event_object_table = 'investment_goal_contributions'
      AND trigger_name = 'investment_contribution_update_goal'
  ) THEN
    CREATE TRIGGER investment_contribution_update_goal
      AFTER INSERT OR DELETE ON public.investment_goal_contributions
      FOR EACH ROW
      EXECUTE FUNCTION public.update_investment_goal_amount();
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE trigger_schema = 'public'
      AND event_object_table = 'investment_goal_contributions'
      AND trigger_name = 'investment_contributions_updated_at'
  ) THEN
    CREATE TRIGGER investment_contributions_updated_at
      BEFORE UPDATE ON public.investment_goal_contributions
      FOR EACH ROW
  ...
  IF NOT EXISTS (
    ...
    FROM information_schema.triggers
    WHERE trigger_schema = 'public'
      AND event_object_table = 'investment_goals'
      AND trigger_name = 'investment_goals_updated_at'
  ) THEN
