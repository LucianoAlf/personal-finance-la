...
export async function getDefaultAIConfig(supabase, userId) {
  const { data, error } = await supabase.from('ai_provider_configs').select('*').eq('user_id', userId).eq('is_default', true).eq('is_validated', true).single();
...
export async function callChat(config, messages) {
...
async function callClaude(config, messages) {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model: config.model,
      system,
      messages: nonSystem.length ? nonSystem : [
        {
          role: 'user',
          content: 'Olá'
        }
...
export async function callVision(config, imageDataUrl, systemPrompt, userText) {
