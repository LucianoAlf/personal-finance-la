...
serve(async (req) => {
  ...
  try {
    ...
    if (!users || users.length === 0) {
      ...
      return new Response(
        JSON.stringify({ message: 'Nenhum usuário ativo', results: [] }),
        { headers: { 'Content-Type': 'application/json' } }
      );
    }
 ...
 */
async function processUserNotifications(supabase: any, userId: string) {
 ...
 * 1. Verificar contas vencendo em 3 dias
 */
async function checkUpcomingBills(supabase: any, userId: string) {
  ...
  const { data: bills, error } = await supabase
    .from('payable_bills')
    .select('description, amount, due_date, provider')
    .eq('user_id', userId)
    .eq('status', 'pending')
  ...
  return { type: 'upcoming_bills', message };
}

/**
 * 2. Verificar status do orçamento
 */
async function checkBudgetStatus(supabase: any, userId: string) {
  const now = new Date();
  ...
  const { data: preferences } = await supabase
    .from('notification_preferences')
    .select('budget_alert_thresholds, budget_alert_cooldown_hours')
    .eq('user_id', userId)
    .single();
  ...
  const { data: budgetGoal } = await supabase
  ...
  await supabase
    ...
    .insert({
      ...
      channel: 'whatsapp',
      status: 'sent',
      metadata: { threshold: reachedThreshold, percentage: percentage.toFixed(1) }
    });

  console.log(`[checkBudgetStatus] ✅ Orçamento em ${percentage.toFixed(1)}% (threshold: ${reachedThreshold}%)`);
  return { type: 'budget_alert', message };
}

/**
 * 3. Verificar metas alcançadas
 */
async function checkAchievedGoals(supabase: any, userId: string) {
  ...
  const { data: goals, error } = await supabase
    .from('financial_goals')
    .select('name, target_amount, current_amount, type')
    .eq('user_id', userId)
    .eq('status', 'completed')
  ...
  return { type: 'goals_achieved', message };
}

/**
 * 4. Verificar dividendos recebidos
 */
async function checkDividends(supabase: any, userId: string) {
  ...
  const { data: dividends, error } = await supabase
    ...
    .select('investment_id, amount, investments(ticker)')
    .eq('user_id', userId)
    .eq('type', 'dividend')
  ...
  return { type: 'dividends_received', message };
}
